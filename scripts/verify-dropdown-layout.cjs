...
const { ROOT, ensureElectron, createRendererWindow, captureVisual, finishElectron } = require('./lib/electron-renderer-fixture.cjs');
ensureElectron(__filename);

(async () => {
  const win = await createRendererWindow({ width: 900, height: 800 });
  await win.webContents.executeJavaScript(`(() => {
    ...
    menu.querySelector('#loopTriggerRow').hidden = false;
    menu.querySelector('#loopTrigger').value = 'after-turn';
  ...
  for (const theme of ['dark', 'light']) for (const zoom of [1, 1.5]) {
    ...
    const measured = await win.webContents.executeJavaScript(`(() => {
      const select = document.getElementById('loopTrigger'), label = document.getElementById('loopTriggerRow');
    ...
    for (const id of ['loopTrigger', 'goalBackend']) {
      ...
      await captureVisual(win, `outputs/dropdown-layout/${theme}-${zoom}-${id}.png`);
  ...
  await win.webContents.executeJavaScript(`(() => {
    ...
    document.getElementById('loopTrigger').showPicker();
  })()`, true);
  const translated = await win.webContents.executeJavaScript(`({ value: document.getElementById('loopTrigger').value,
    fits: [...document.getElementById('loopTrigger').options].every(option => { const r = option.getBoundingClientRect(); return r.left >= 0 && r.right <= innerWidth; }) })`);
  assert.deepEqual(translated, { value: 'after-turn', fits: true });
  await captureVisual(win, 'outputs/dropdown-layout/light-1.5-zh.png');
