...
const { bridgePort, startBridge, stopBridge, recoveryInputAllowed, installBridgeRuntimeDependencies } = await import('../src/main/bridge.js');
...
beforeAll(async () => {
  ...
  input.installInputAuthority({ automaticFinishEnabled: goal.automaticFinishEnabled, recoveryAllowed: recoveryInputAllowed });
...
beforeEach(async () => {
  await writeDurableNow('session-input', encodeInputState([]));
    ...
    it.each(['none', 'previous-turn', 'native-tool', 'request-only', 'current-turn'] as const)(
      ...
      try {
        ...
        if (row) {
          ...
          expect((await input.listInputs()).find(item => item.id === row.id)?.followupPermit?.kind === 'recovery').toBe(eligible);
...
it.each(['gpt-6-pro', 'gpt-5.6-sol'])('carries settled Thinking failed through HTTP, recording and one queued send (%s)', async model => {
  ...
  try {
    ...
    expect(claim.body.input).toMatchObject({ id: first.id, followupPermit: expect.objectContaining({ kind: 'recovery' }) });
...
it('ignores unshipped development recovery fields without MCP proof but still delivers a real final', async () => {
  ...
  goal.restoreGoalReplies({ version: 1, savedAt: Date.now(), replies: [{ conversationId, sessionId: session.id,
    replyId: 'silence:restored', turnId: 'g-silence-restored', recovery: { proof: {
      kind: 'recovered-silence', conversationId, turnId: 'restored-turn', headSeq: start!.seq, mcpCallSeq: 0, modelClass: 'pro'
...
it('revokes Thinking-failed recovery when durable work lands after the failure boundary', async () => {
  const conversationId = randomUUID();
  ...
  await attributedMcp(conversationId);
  await post('/events', { conversationId, events: [
    { kind: 'turn_end', turnId: 'failed-then-resumed', outcome: 'failed', reason: 'thinking_failed', time: Date.now() + 1 },
    { kind: 'assistant_message', turnId: 'failed-then-resumed', messageId: 'resumed-after-failure', text: 'Work resumed',
      state: 'streaming', activeNow: true, time: Date.now() + 2 }
  ] });
  expect((await input.listInputs()).find(entry => entry.id === row.id)?.followupPermit).toBeUndefined();
  expect(await input.pendingBrowserInputs()).toEqual([]);
...
it.each(['open', 'stalled', 'final-during-listen', 'failure-during-listen'])('files one durable after-turn ticket only after the silence refresh ACK (%s)', async boundary => {
  ...
  try {
    ...
    expect(await input.pendingBrowserInputs()).toEqual([]);
    await post(`/status?repaired=${repair.token}&repairAction=reloaded`, { openConversations: [conversationId] });
    expect((await input.listInputs()).find(row => row.id === first.id)?.followupPermit).toMatchObject({ kind: 'recovery', grant: { proof: { turnId: 'silence-turn' } } });
    input.resetInputForTests();
    expect(await input.pendingBrowserInputs()).toEqual([expect.objectContaining({ id: first.id, recoveryTurnId: 'silence-turn' })]);
    const claim = { id: first.id, owner: 'refreshed-page', conversationId };
    expect((await post('/input/claim', { ...claim, recoveryBusyTurnId: 'silence-turn' })).body.ok).toBe(true);
    expect(await input.pendingBrowserInputs()).toEqual([]);
    ...
    if (boundary !== 'final-during-listen') {
      ...
      expect(permit?.kind).toBe('recovery');
      const until = permit?.kind === 'recovery' ? permit.grant.notBefore! : 0;
      now = until - 1;
    ...
    expect((await post('/input/fail', { ...claim, reason: 'pickup_withdrawn_before_send', error: 'After-turn pickup was withdrawn before Send.' })).body.ok).toBe(true);
    input.resetInputForTests();
    expect(await input.pendingBrowserInputs()).toHaveLength(1);
    expect((await post('/input/claim', { ...claim, requiresAuthorization: true })).body.input?.id).toBe(first.id);
...
it('gives a recovered checkpoint to queued user input before synthesizing Loop work', async () => {
  const bridge = await import('../src/main/bridge.js');
  let now = Date.now();
  const clock = vi.spyOn(Date, 'now').mockImplementation(() => now);
  try {
    const conversationId = randomUUID();
    const session = await createSession({ title: 'User input wins recovery', conversationId });
    const config = defaultConfig();
    await saveConfig({ ...config, goal: { ...config.goal, enabled: true, mode: 'loop', loopBackend: 'chatgpt' } });
    await goal.setGoalSwitchNow(conversationId, 'loop', true);
    await goal.setGoalLoopTriggerNow(conversationId, 'after-turn');
    ...
    expect(repair).toBeDefined();
    await post(`/status?repaired=${repair.token}&repairAction=reloaded`, { openConversations: [conversationId] });
    expect((await input.listInputs()).find(row => row.id === queued.id)?.followupPermit).toMatchObject({
      kind: 'recovery', grant: { proof: { turnId: 'priority-turn', kind: 'recovered-silence' } }
    });
    expect(goal.goalPendingReplyFor(conversationId)).toBeNull();
  } finally { clock.mockRestore(); }
});

it('transfers an unspent Loop recovery to user input queued after the synthetic obligation already exists', async () => {
  const bridge = await import('../src/main/bridge.js');
  let now = Date.now();
  const clock = vi.spyOn(Date, 'now').mockImplementation(() => now);
  try {
    const conversationId = randomUUID();
    const session = await createSession({ title: 'Late user recovery priority', conversationId });
    const config = defaultConfig();
    await saveConfig({ ...config, goal: { ...config.goal, enabled: true, mode: 'loop', loopBackend: 'chatgpt' } });
    await goal.setGoalSwitchNow(conversationId, 'loop', true);
    await goal.setGoalLoopTriggerNow(conversationId, 'after-turn');
    ...
    expect(repair).toBeDefined();
    await post(`/status?repaired=${repair.token}&repairAction=reloaded`, { openConversations: [conversationId] });
    expect(goal.goalPendingReplyFor(conversationId)?.recovery).toBeTruthy();
    const queued = await input.enqueueInput({ ...message(session.id, 'off'), mode: 'after-turn' });
    expect((await input.listInputs()).find(row => row.id === queued.id)?.followupPermit).toMatchObject({
      kind: 'recovery', grant: { proof: { turnId: 'late-priority-turn' } }
    });
    expect(goal.goalPendingReplyFor(conversationId)).toBeNull();
  } finally { clock.mockRestore(); }
});

it('refuses synthetic recovery spend even when retiring its duplicate projection cannot persist', async () => {
  const bridge = await import('../src/main/bridge.js');
  let now = Date.now();
  const clock = vi.spyOn(Date, 'now').mockImplementation(() => now);
  let failGoalWrites = false;
  const renameFile = fs.rename.bind(fs);
  const rename = vi.spyOn(fs, 'rename').mockImplementation(async (from, to) => {
    if (failGoalWrites && String(to).replaceAll('\\', '/').endsWith('/state/goal-replies.json')) {
      throw Object.assign(new Error('simulated goal-replies persistence failure'), { code: 'EIO' });
    }
    return renameFile(from, to);
  });
  try {
    const conversationId = randomUUID();
    const session = await createSession({ title: 'Recovery cleanup failure', conversationId });
    const config = defaultConfig();
    await saveConfig({ ...config, goal: { ...config.goal, enabled: true, mode: 'loop', loopBackend: 'chatgpt' } });
    await goal.setGoalSwitchNow(conversationId, 'loop', true);
    await goal.setGoalLoopTriggerNow(conversationId, 'after-turn');
    ...
    expect(repair).toBeDefined();
    await post(`/status?repaired=${repair.token}&repairAction=reloaded`, { openConversations: [conversationId] });
    const synthetic = goal.goalPendingReplyFor(conversationId);
    expect(synthetic?.recovery).toBeTruthy();

    failGoalWrites = true;
    const queued = await input.enqueueInput({ ...message(session.id, 'off'), mode: 'after-turn' });
    expect((await input.listInputs()).find(row => row.id === queued.id)?.followupPermit).toMatchObject({
      kind: 'recovery', grant: { proof: { turnId: 'cleanup-failure-turn' } }
    });
    // The cleanup write failed and rolled Goal back to pending, so both durable projections are
    // deliberately visible at once. The final spend fence must resolve the conflict in favor of
    // explicit user input rather than relying on best-effort cleanup having succeeded.
    expect(goal.goalPendingReplyFor(conversationId)?.replyId).toBe(synthetic!.replyId);
    const draft = await post('/goal/draft', {
      conversationId,
      turnId: synthetic!.turnId,
      clientId: 'cleanup-failure-page',
      terminalRequired: true
    });
    expect(draft.status).toBe(409);
    expect(draft.body).toMatchObject({ error: 'explicit_input_precedes_goal', retryable: true });
    expect(goal.goalPendingReplyFor(conversationId)?.replyId).toBe(synthetic!.replyId);
  } finally {
    failGoalWrites = false;
    rename.mockRestore();
    goal.resetGoalStateForTests();
    await writeDurableNow(goal.GOAL_REPLIES_STATE, goal.snapshotGoalReplies());
    clock.mockRestore();
  }
});
...
it.each(['queued', 'claimed', 'tool', 'settled-failure'])('withdraws a silence ticket on new work and rearms refresh (%s)', async change => {
  ...
  try {
    ...
    expect((await input.listInputs()).find(row => row.id === first.id)?.followupPermit).toBeUndefined();
...
it('freezes image injection from staged originals with replay, receipt, and browser isolation', async () => {
  ...
  const { readToolImageProjections } = await import('../src/main/session/input-image-projection.js');
  const { readEvents } = await import('../src/main/session/store.js');
  const conversationId = randomUUID();
  ...
  const [first, duplicate] = await Promise.all([
    input.enqueueInput(authored),
    input.enqueueInput(authored)
  ]);
  expect(duplicate.projectedImages).toEqual(first.projectedImages);
  const result = await handlers.get('sessions:send')!(null, authored);
  expect(result.ok).toBe(true);
  expect(result.data).toMatchObject({ attachments: [attachment], attachmentDelivery: 'tool-image-projection', transportIntent: 'tool' });
  expect(result.data.projectedImages[0]).toMatchObject({ name: 'full-resolution.webp', size: expect.any(Number), sha256: expect.stringMatching(/^[a-f0-9]{64}$/) });
  const dataUrl = (await readToolImageProjections(result.data.projectedImages))[0]!.dataUrl;
  expect(result.data.projectedImages[0].name).toBe('full-resolution.webp');
  expect(await sharp(Buffer.from(dataUrl.split(',')[1]!, 'base64')).metadata()).toMatchObject({ width: 1600, height: 800 });
  input.resetInputForTests();
  const replay = (await handlers.get('sessions:send')!(null, authored)).data;
  expect((await readToolImageProjections(replay.projectedImages))[0]!.dataUrl).toBe(dataUrl);
  expect(await input.pendingBrowserInputs()).toEqual([]);
  ...
  expect((await input.listInputs())[0]).toMatchObject({ state: 'sent', historyRecorded: true });
  expect((await input.listInputs())[0]!.projectedImages).toBeUndefined();
  ...
  expect(history[0]!.attachmentDelivery).toBe('tool-image-projection');
...
it('rejects image admission when the session changes during normalization', async () => {
  ...
  const projections = await import('../src/main/session/input-image-projection.js');
  const { default: sharp } = await import('sharp');
  const conversationId = randomUUID();
  ...
  const store = path.join(directory, 'input-attachments');
  const projectionFiles = async () => (await fs.readdir(store)).filter(name => name.endsWith('.json') && name !== `${file.id}.json`);
  const before = await projectionFiles();
  let release!: () => void;
  const gate = new Promise<void>(resolve => { release = resolve; });
  const normalize = projections.createToolImageProjections;
  const held = vi.spyOn(projections, 'createToolImageProjections').mockImplementation(async files => { await gate; return normalize(files); });
  try {
    const pending = input.enqueueInput({ ...message(session.id, 'off'), mode: 'auto', attachments: [file], attachmentDelivery: 'tool-image-projection' });
    ...
    expect(await projectionFiles()).toEqual(before);
...
it('retires a late-confirmed cancelled desktop send after two minutes even as the only managed chat', async () => {
  ...
  try {
    ...
    await writeDurableNow('session-input', encodeInputState([previous, { ...previous, id: randomUUID(), sessionId: session.id,
