...
it.each(['exec_command', 'write_stdin'] as const)('delivers terminal corrections through the actual %s structured wire without changing process output', async name => {
  ...
  try {
    ...
    expect(plain.result.structuredContent).not.toHaveProperty('delivery_context');
    ...
    expect(response.result.structuredContent).toEqual({ ...plain.result.structuredContent,
      delivery_context: expect.stringContaining('ACK_G7391_CORRECTION') });
    expect(response.result.structuredContent.output).toBe('');
    expect(response.result.structuredContent.delivery_context).toContain('CONTROLLED_BACKGROUND_NOTICE');
...
it('preserves a nonzero terminal exit and leaves nested correction delivery with the outer result', async () => {
  ...
  expect(direct.result.structuredContent).toMatchObject({ exit_code: 7, output: 'process failure', delivery_context: expect.stringContaining('NONZERO_CORRECTION') });
  ...
  expect(child).not.toHaveProperty('delivery_context');
...
it('projects corrections for other Core structured results without changing the empty worker family', async () => {
  ...
  try {
    ...
    expect(delivered.result.structuredContent).toEqual({ ...before.result.structuredContent, delivery_context: expect.stringContaining('CORE_STATUS_CORRECTION') });
...
afterEach(async () => {
  ...
  const config = getConfig();
  await saveConfig({ ...config, multiAgent: { ...config.multiAgent, allowUnattributedCalls: true, allowUnattributedComputerControl: false } });
});
afterAll(async () => {
...
it.runIf(process.platform === 'win32').each([true, false])('routes sky through Desktop MCP with attributed=%s, retaining observation and live permissions', async attributed => {
  const who = attributed ? await identity() : { requestId: `wfr_${randomUUID().replaceAll('-', '')}`, session: { id: null } };
  if (!attributed) {
    const config = getConfig();
    await saveConfig({ ...config, multiAgent: { ...config.multiAgent, allowUnattributedCalls: true, allowUnattributedComputerControl: true } });
  }
  ...
  const observed = await rpc('tools/call', { name: 'exec', arguments: { code: 'const window = (await sky.list_windows())[0]; const state = await sky.get_window_state({window,include_text:true}); await sky.click({window,element_index:0}); nodeRepl.write({window:state.window,tree:state.accessibility.tree}); text("accepted");' } }, who.requestId, 'desktop');
  ...
  expect(text(observed)).toContain('accepted');
