...
import { readToolImageProjections, validateInputImages } from '../src/main/session/input-image-projection.js';
...
it('carries validated user image bytes through an exact-session MCP result and acknowledges the next same-turn call', async () => {
  ...
  try {
    ...
    const authored = { id: randomUUID(), sessionId: session.id, text: 'Use this image', attachments: [attachment], attachmentDelivery: 'tool-image-projection' as const, mode: 'auto' as const, dueAt: 0, model: null, reasoningEffort: null };
    const input = await enqueueInput(authored);
    expect((await enqueueInput(authored)).id).toBe(input.id);
    const injectedBytes = (await readToolImageProjections(input.projectedImages!))[0]!.dataUrl.split(',')[1]!;
    const additional = [];
