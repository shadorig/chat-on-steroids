...
vi.mock('../src/main/browser-startup.js', () => ({
  installBrowserStartupPresence: vi.fn(),
  wakeBrowserUrl: recoveryBrowserWake
}));
...
describe('automatic compaction', () => {
  ...
  it('withdraws an already filed automatic ticket before its send after selecting Astra', async () => {
    ...
    await withThreshold(10_000, async () => {
      ...
      const send = await request('POST', '/compact', { body: { conversationId, token: ticket.token, action: 'source-claim' } });
  ...
  it('suspends Goal, Loop and compaction for a chat the user blocked, until it is released', async () => {
    ...
    try {
      await withThreshold(10_000, async () => {
        ...
        expect(activity.body.goal).toMatchObject({ effectiveEnabled: false, own: true, blocked: 'blocked', pending: null });
  ...
  it('durably ends an automatic ticket when the source page proves the handoff never reached Send', async () => {
    ...
    const token = filed.body.token as string;
    expect((await request('POST', '/compact', { body: { conversationId, token, action: 'source-claim' } })).body.allowed).toBe(true);

    const lost = await request('POST', '/compact', { body: { conversationId, token, action: 'source-release' } });
    ...
    expect((await request('POST', '/compact', { body: { conversationId, token, action: 'source-arm' } })).status).toBe(409);
  });
  ...
  it('does not immediately refile a rejected automatic compaction in the same working turn', async () => {
    ...
    await withThreshold(10_000, async () => {
      ...
      const lost = await request('POST', '/compact', {
        body: { conversationId, token: first!.token, action: 'source-release' }
      });
      expect(lost.status).toBe(200);
      expect(continuationForSession(sessionId)).toBeNull();
  ...
  it('reloads one automatic ticket at 5-minute checkpoints while the brief is being written', async () => {
    ...
    try {
      ...
      expect((await request('POST', '/compact', { body: { conversationId, token, action: 'source-claim' } })).body.allowed).toBe(true);
      expect((await request('POST', '/compact', { body: { conversationId, token, action: 'source-arm' } })).body.armed).toBe(true);
...
describe('delivering a bootstrap', () => {
  async function prepareExpiredDocumentOwnedRevivalRestoreFixture(options: {
    ...
    const resumeCommand = await queueResume(resume.sessionId, resume.token)!;
  ...
  it('opens the chat, hands the brief to the page that redeems it, and forgets it on success', async () => {
    ...
    const command = await queueResume(sessionId, token);
  ...
  it('leases Project entry only to its exact durable source and returns the native navigation intent', async () => {
    ...
    const command = await queueResume(session.id, ticket)!;
    await waitForOpened(1);
  ...
  it('protects a resume destination before the browser opener can record a shadow session', async () => {
    ...
    const command = await queueResume(sessionId, continuation)!;
  ...
  it('lets a reloaded page adopt a resume until one arms the click, and refuses every page afterwards', async () => {
    ...
    const claimed = await request('POST', '/compact', { body: { token, commandId: command.id, client: 'tab-2', action: 'destination-claim' } });
    expect(claimed.body.allowed).toBe(true);
    expect((await redeem(command.id, 'tab-3')).text).toContain('the only brief');

    const armed = await request('POST', '/compact', { body: { token, commandId: command.id, client: 'tab-3', action: 'destination-arm' } });
  ...
  it('rejects an unknown continuation action instead of interpreting it as an ordinary compact request', async () => {
    await pair();
    const conversationId = '98989898-8888-4777-8666-555555555555';
    const { token } = await compactedSession(conversationId, 'keep the action grammar exclusive');
    const before = continuationByToken(token);

    const reply = await request('POST', '/compact', {
      body: { conversationId, token, action: 'checkpoint-something-else' }
    });

    expect(reply.status).toBe(400);
    expect(reply.body.error).toBe('bad_checkpoint_action');
    expect(continuationByToken(token)).toEqual(before);
  });
  ...
  it('offers the brief to a fresh chat once the page proves it lost the draft before Send', async () => {
    ...
    expect((await request('POST', '/compact', { body: { token, commandId: command.id, client: 'tab-1', action: 'destination-claim' } })).body.allowed).toBe(true);
    expect((await request('POST', '/compact', { body: { token, commandId: command.id, client: 'tab-1', action: 'destination-arm' } })).body.armed).toBe(true);
    expect((await request('POST', '/commands/redeem', { body: { id: command.id, client: 'tab-2' } })).status).toBe(409);

    const before = opened.length;
    const lost = await request('POST', '/compact', { body: { token, commandId: command.id, client: 'tab-1', action: 'destination-release' } });
    expect(lost.status).toBe(200);
    ...
    expect((await request('POST', '/compact', { body: { token: 'ffffffffffffffffffffffffffffffff', action: 'destination-release' } })).status).toBe(400);
  });
  ...
  it('aborts a resume whose commit the session layer refused, so chat A is not left waiting on it', async () => {
    ...
    expect((await request('POST', '/compact', { body: { token, commandId: command.id, client: 'tab-b', action: 'destination-claim' } })).body.allowed).toBe(true);
    expect((await request('POST', '/compact', { body: { token, commandId: command.id, client: 'tab-b', action: 'destination-arm' } })).body.armed).toBe(true);
  ...
  it('arms the resumed chat the moment the session moves onto it', async () => {
    ...
    expect((await request('POST', '/compact', { body: { token, commandId: command.id, client: 'tab-b2', action: 'destination-claim' } })).body.allowed).toBe(true);
    expect((await request('POST', '/compact', { body: { token, commandId: command.id, client: 'tab-b2', action: 'destination-arm' } })).body.armed).toBe(true);
  ...
  it('names the chat it opened after the work, not after the bootstrap it typed', async () => {
    ...
    const command = await queueResume(source.id, await readyContinuation(source.id, 'carry on'))!;
  ...
  it('ends a failed bootstrap instead of retrying it, and has no way to postpone one', async () => {
    ...
    const second = await queueResume(againId, againToken)!;
  ...
  it.each(['worker', 'resume'] as const)('adds setup only to a new worker, never a resumed chat (%s)', async kind => {
    ...
    if (kind === 'worker') {
      ...
      const pending = await queueResume(source.id, continuation)!;
  ...
  it('captures the sending Project without an earlier activity poll', async () => {
    ...
    const project = 'g-p-11111111222233334444555555555555';
    const result = await request('POST', '/compact', { body: { conversationId: id, token: opened.token, action: 'source-claim', project } });
  ...
  it('replays a committed sent ACK after a crash between worker fsync and command receipt without duplicating the wake', async () => {
    ...
    resetSwarm();
    await resetBridgeForTests();
    // A process crash cannot run the live bridge listener that resetSwarm() uses in this test
    // process. Restore the exact command ledger captured at the crash cut before startup rebuilds
    // memory, otherwise test cleanup itself durably cancels the leased revival we mean to replay.
    await writeDurableNow('bridge-commands', durableCommands);
  ...
  it('starts a worker while an unrelated resume awaits its exact receipt', async () => {
    ...
    await attachSummary(continuation.token, SAMPLE_BRIEF);
    const resume = await queueResume(source.id, continuation.token)!;
    await waitForOpened(1);
  ...
  it('restores a resume when its continuation WAL is restored first', async () => {
    ...
    const command = await queueResume(source.id, continuation)!;
    await waitForOpened(1);
    await flushDurable();
    ...
    await queueResume(source.id, continuation);
  ...
  it('runs the full lifecycle cleanup when the command queue overflows', async () => {
    ...
    for (let n = 0; n < 20; n++) await queueResume(`overflow-session-${n}`, `overflow-handoff-${n}`);
  ...
  it('keeps a dropped worker transport durable until the failed broker state is durable', async () => {
    ...
    try {
      ...
      for (let n = 0; n < 20; n++) await queueResume(`crash-order-session-${n}`, `crash-order-token-${n}`);
      await gate.entered;
...
describe('targeted open', () => {
  it('opens the fresh chat the instant a resume is queued, with no tab and no timer involved', async () => {
    ...
    const command = await queueResume('session-open', 'handoff-open')!;
  ...
  it('ends a browser-open rejection immediately rather than blocking the command queue', async () => {
    ...
    await queueResume('session-nobrowser', 'handoff-nobrowser');
  ...
  it('ends a command outright when this process cannot open a browser at all', async () => {
    ...
    await queueResume(sessionId, token);
  ...
  it('collapses repeated presses for one session into one job, one command and one tab', async () => {
    ...
    const first = await queueResume('session-once', 'handoff-1')!;
    const second = await queueResume('session-once', 'handoff-1')!;
    const third = await queueResume('session-once', 'handoff-1')!;
  ...
  it('supersedes a queued resume in place when the same session is compacted again', async () => {
    ...
    const first = await queueResume(older.sessionId, older.token)!;
    ...
    const second = await queueResume(older.sessionId, newerToken)!;
  ...
  it('withdraws a cancelled resume so no tab opens for it afterwards', async () => {
    ...
    await pair();
    const command = await queueResume('session-cancel', 'handoff-cancel')!;
    await waitForOpened(1);

    expect(await cancelResume('session-cancel')).toBe(true);
    expect(pendingCommands()).toEqual([]);
...
describe('a worker chat that never opens', () => {
  ...
  it('does not reopen a sibling when a lost-ACK bootstrap retires', async () => {
    ...
    try {
      ...
      await vi.waitFor(() => expect(pendingCommands().some(command => command.id === first.id)).toBe(false));
    } finally {
      vi.useRealTimers();
    }
  });
...
describe('unattributed activity recovery', () => {
  ...
  for (const model of ['GPT-6 Pro', 'GPT-5.6 Pro', 'gpt-5-6-pro', 'gpt-5-5-pro']) it(`keeps ${model} silent until ten minutes without inventing a Goal final`, async () => {
    ...
    try {
      ...
      await vi.advanceTimersByTimeAsync(CONTINUATION_SILENCE_MS - 1_001);
      await sweepStaleSwarm(Date.now());
      expect(await maintenance()).toBeNull();
  ...
  it.each(['late picker', 'exact call', 'picker after start in same batch'])('resolves an initially unknown Pro turn through %s before silence recovery', async proof => {
    ...
    try {
      ...
      await vi.advanceTimersByTimeAsync(evidenceAt + CONTINUATION_SILENCE_MS - Date.now() - 1);
      await sweepStaleSwarm(Date.now());
      expect(await maintenance()).toBeNull();
      await vi.advanceTimersByTimeAsync(1);
      await sweepStaleSwarm(Date.now());
  ...
  it('retires an old synthetic Pro obligation before later model changes can resurrect it', async () => {
    ...
    restoreGoalReplies({ version: 1, savedAt: Date.now(), replies: [{ conversationId: OTHER,
      sessionId: activity.body.sessionId, turnId: 'g-silence-old', replyId: 'silence:old', eventSeq: 0,
      acceptedAt: Date.now(), state: 'pending', recovery: { proof: {
        kind: 'recovered-silence', conversationId: OTHER, turnId: 'pro-old', headSeq: 0, mcpCallSeq: 0, modelClass: 'pro'
      } } }] });
    ...
    await deactivateGoalReplyNow(OTHER);
    await tryActivateGoalReplyNow(OTHER);
  ...
  for (const afterTurn of [false, true]) for (const proof of ['missing', 'exact', 'older', 'new turn']) it(`requires ${proof} canonical Pro final proof without any recent tool call (after-turn ${afterTurn})`, async () => {
    ...
    try {
      await pair();
      const { setGoalLoopTriggerNow, setGoalSwitchNow } = await import('../src/main/goal.js');
      await setGoalSwitchNow(conversationId, 'loop', true);
      if (afterTurn) await setGoalLoopTriggerNow(conversationId, 'after-turn');
  ...
  it('lets only exact MCP work spanning the terminal observation preserve turn activity', async () => {
    await pair();

    const historicalTurn = 'turn-terminal-historical-mcp';
    await events(OTHER, [openTurn(historicalTurn)]);
    await attributed(OTHER, false, Date.now());
    await events(OTHER, [endTurn(historicalTurn, 'completed')]);
    const historicalSession = await findSessionByConversation(OTHER, { requireUnique: true });
    expect(historicalSession).not.toBeNull();
    expect(sessionInputActivity(historicalSession!).exact).toBe(false);

    const spanningTurn = 'turn-terminal-spanning-mcp';
    await events(OTHER, [openTurn(spanningTurn)]);
    let release = (): void => {};
    let terminalObserved!: () => void;
    const held = new Promise<void>((resolve) => { release = resolve; });
    const observed = new Promise<void>((resolve) => { terminalObserved = resolve; });
    const call = trackInFlight({
      startedAt: Date.now(),
      transportKey: null,
      agent: null,
      caller: { transportKey: null, requestId: 'wfr-terminal-spanning-mcp', conversationId: OTHER },
      outcome: null,
      evidence: emptyEvidence()
    }, async () => {
      await events(OTHER, [endTurn(spanningTurn, 'completed')]);
      terminalObserved();
      await held;
    });

    await observed;
    const spanningSession = await findSessionByConversation(OTHER, { requireUnique: true });
    expect(spanningSession).not.toBeNull();
    expect(spanningSession!.activeTurnId).toBeNull();
    expect(sessionInputActivity(spanningSession!)).toMatchObject({ exact: true, turnId: spanningTurn });
    release();
    await call;
  });
  ...
  it('refuses to draft for a chat whose tools ran within the last minute, then drafts once it is quiet', async () => {
    ...
    try {
      ...
      expect(goalPendingReplyFor(OTHER)).toMatchObject({
        replyId: 'turn:turn-false-end',
        source: { kind: 'turn', turnId: 'turn-false-end' },
        turnId: 'turn-false-end'
      });
  ...
  it('files one opted-in Pro Loop ticket on refresh, defers native busy, and revokes it on MCP work', async () => {
    ...
    try {
      await pair();
      await goal.setGoalSwitchNow(OTHER, 'loop', true);
      await goal.setGoalLoopTriggerNow(OTHER, 'after-turn');
      await events(OTHER, [{ kind: 'model_selection', model: 'gpt-5.6-pro', reasoningEffort: 'pro', time: Date.now() }, openTurn('pro-loop-after-turn')]);
      await attributed(OTHER, false, Date.now());
      ...
      await vi.advanceTimersByTimeAsync(CONTINUATION_SILENCE_MS - 1);
      expect(await maintenance()).toBeNull();
      expect(goalPendingReplyFor(OTHER)).toBeNull();
      await vi.advanceTimersByTimeAsync(1);
      await sweepStaleSwarm(Date.now());
      ...
      const ticket = goalPendingReplyFor(OTHER)!;
      expect(ticket).toMatchObject({ recovery: { proof: { turnId: 'pro-loop-after-turn', modelClass: 'pro', kind: 'recovered-silence' } } });
      ...
      expect(deferred.recovery?.notBefore).toBe(Date.now() + 5 * 60_000);
  ...
  it.each(['completed', 'thinking_failed'])('lets opted-in Pro Loop draft at its confirmed %s boundary', async kind => {
    ...
    vi.useFakeTimers();
    try {
      await pair(); await goal.setGoalSwitchNow(OTHER, 'loop', true); await goal.setGoalLoopTriggerNow(OTHER, 'after-turn');
      const turn = `pro-loop-${kind}`;
      await events(OTHER, [{ kind: 'model_selection', model: 'gpt-5.6-pro', reasoningEffort: 'pro', time: Date.now() }, openTurn(turn)]);
      ...
      if (kind === 'thinking_failed') {
        ...
        await sweepStaleSwarm(Date.now());
        const repair = await maintenance();
        expect(repair?.reason).toBe('failed-view');
...
describe('the goal loop over the bridge', () => {
  ...
  it('repairs Goal onto the exact pre-fix resume-shadow chat and can draft there', async () => {
    ...
    expect(feed.body.goal).toMatchObject({ effectiveEnabled: true, objective: 'finish the release from the resumed prime chat' });
  ...
  it('leaves the loop on for the prime and off for every worker it spawns', async () => {
    ...
    expect((await request('GET', `/activity?conversationId=${worker}`)).body.goal.effectiveEnabled).toBe(false);
    // A chat that belongs to no agent in the run is an ordinary chat and keeps the loop.
    expect((await request('GET', `/activity?conversationId=${solo}`)).body.goal.effectiveEnabled).toBe(true);
  ...
  it('drafts once, hands the message over once, and forgets it on acknowledgement', async () => {
    ...
    try {
      ...
      expect(calls).toBe(1);
      expect(feed.body.goal.draft.reply).toBe('what about the tests');
  ...
  it('lets the page set the two switches it owns, and nothing else', async () => {
    ...
    expect(reply.body.goal).toMatchObject({ configuredEnabled: false, effectiveEnabled: false, hasKey: true });
  ...
  it('runs Goal or Loop but never both, and keeps the mode when a switch goes off', async () => {
    ...
    expect(loop.body.goal).toMatchObject({ configuredEnabled: true, effectiveEnabled: true, mode: 'loop' });
    // The page is told the mode as well, which is the whole reason it can never draw both on.
    expect((await request('GET', '/settings')).body.goal).toMatchObject({ configuredEnabled: true, effectiveEnabled: true, mode: 'loop' });
  ...
  it('stops a chat that switched itself off, and keeps its goal for when it starts again', async () => {
    ...
    expect(inherited.body.goal).toMatchObject({ effectiveEnabled: false, own: false, objective: 'port the module' });
    ...
    expect(off.status).toBe(200);
    expect(off.body.goal).toMatchObject({ configuredEnabled: false, effectiveEnabled: false, own: true });
    const stopped = await request('GET', `/activity?conversationId=${chat}`);
    expect(stopped.body.goal).toMatchObject({ effectiveEnabled: false, own: true, objective: 'port the module' });
    ...
    expect(on.body.goal).toMatchObject({ configuredEnabled: true, effectiveEnabled: true, own: true, mode: 'loop' });
    const realFetch = globalThis.fetch;
    globalThis.fetch = (async () =>
      Response.json({
  ...
  it('pins the mode a goal was written in as that chat’s own switch', async () => {
    ...
    expect(looped.body).toMatchObject({ objective: 'build the voxel sandbox', configuredEnabled: true, effectiveEnabled: true, mode: 'loop' });
    ...
    expect(backToGoal.body).toMatchObject({ configuredEnabled: true, effectiveEnabled: true, mode: 'goal' });
    expect(goalDrivingMode(chat)).toBe('goal');
    ...
    expect(cleared.body).toMatchObject({ objective: '', configuredEnabled: false, effectiveEnabled: false });
  ...
  it('stores the Goal switch against the chat whose composer sent it', async () => {
    ...
    expect((await request('GET', `/activity?conversationId=${driven}`)).body.goal.effectiveEnabled).toBe(true);
    expect((await request('GET', `/activity?conversationId=${other}`)).body.goal.effectiveEnabled).toBe(true);

    const off = await request('POST', '/settings', { body: { conversationId: driven, goal: false } });
    expect(off.status).toBe(200);
    expect(off.body.goal).toMatchObject({ configuredEnabled: false, effectiveEnabled: false });
    expect((await request('GET', `/activity?conversationId=${driven}`)).body.goal.effectiveEnabled).toBe(false);
    // The chat that said nothing is untouched, and so is the app-wide default it follows.
    expect((await request('GET', `/activity?conversationId=${other}`)).body.goal.effectiveEnabled).toBe(true);
    expect(getConfig().goal.enabled).toBe(true);
    expect((await request('GET', '/settings')).body.goal.configuredEnabled).toBe(true);
  ...
  it.each(['browser', 'native'])('re-arms the newest completed reply when %s edits its Loop objective during compaction', async surface => {
    ...
    await deactivateGoalReplyNow(chat);
    ...
    await request('POST', '/events', {
      body: {
        ...
        events: [
          ...
          { kind: 'turn_end', time: Date.now(), turnId: 'switch-ticket-turn', outcome: 'completed' }
        ]
      }
    });
  ...
  it('refuses to hold a goal for a worker chat, and says so on the feed', async () => {
    ...
    expect(feed.body.goal).toMatchObject({ effectiveEnabled: false, objective: '', blocked: 'worker' });
  });
  ...
  it('writes the opening message of a chat that does not exist yet', async () => {
    ...
    try {
      ...
      expect(opened.body).toEqual({
        reply: 'rewrite the parser in rust',
        model: 'deepseek/deepseek-v4-flash'
      });
  ...
  it('reports the settings without a conversation to report them for', async () => {
    ...
    expect((await request('GET', '/settings')).body.goal.configuredEnabled).toBe(false);
...
describe('app requests to stop one exact active turn', () => {
  it('does not present a persisted unfinished turn as live after recorder restart', async () => {
    ...
    resetRecorderForTests();
    await resetBridgeForTests(); // A process restart clears the live activity owner too.
  ...
  it('keeps a durable Stop visible while an unrelated command mutation is still fsyncing', async () => {
    const { stopSessionTurn } = await import('../src/main/bridge.js');
    const { promises: fs } = await import('node:fs');
    const conversationId = 'e1212121-aaaa-4bbb-8ccc-111111111111';
    const sessionId = await active(conversationId);
    await stopSessionTurn(sessionId, 'stop-turn-one');
    ...
    const unrelated = queueResume('unrelated-stop-status-session', 'unrelated-stop-status-token');
    await gate.entered;
    try {
      const status = await request('GET', '/status');
      expect(status.body.stopTurns).toEqual([
        expect.objectContaining({ conversationId, turnId: 'stop-turn-one' })
      ]);
    } finally {
      gate.release();
      await unrelated;
