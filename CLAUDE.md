...
commit, push, tag, or release, run `corepack pnpm run verify:privacy`. The versioned Git hooks installed by
`corepack pnpm run hooks:install` enforce the same policy for Claude-created commits.
